...
class CRM_Memoriamigration_Migrator {
  ...
  public function run() {
    ...
    try {
      ...
      // (We migreren per lid zodat we dat lid als gemigreerd kunnen markeren en we daar opnieuw kunnen beginnen als er iets misgaat)
  ...
  private function getParentGroup() {
    ...
    $customFieldAfdelingGroep = 'custom_' . civicrm_api3('CustomField', 'getvalue', ['name' => 'afdeling_groep', 'return' => 'id']);
    $spgeo = civicrm_api3('Contact', 'getsingle', [
      'contact_id' => $geo_id,
      'return'     => 'contact_id,display_name,' . $customFieldAfdelingGroep,
    ]);

    if ($spgeo[$customFieldAfdelingGroep]) {
      $parentGroupId = $spgeo[$customFieldAfdelingGroep];
    }
    else {
      ...
      if ($parentGroupId) {
        civicrm_api3('Contact', 'create', [
          'contact_id'              => $geo_id,
          $customFieldAfdelingGroep => $parentGroupId,
        ]);
      }
      else {
        return FALSE;
  ...
  private function createGroup($title, $parentGroupId = NULL) {
    ...
    try {
      ...
      if ($ret && !$ret['is_error']) {
        ...
        $this->log("Could not create group: " . $title . " (" . $name . ", parent " . $parentGroupId . ") - " . $ret['error_message']);
        return FALSE;
      }
    }
  }
